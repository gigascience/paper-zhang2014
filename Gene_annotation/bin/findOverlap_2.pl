...
chr1.9  2047    chr1    2       ENSGALT00000015914_GALGA.2,153,153,36125,36277  ENSGALT00000015914_GALGA.3,141,141,36999,37139

  Column 1: the query ID; 
  Column 2: the query size; 
  Column 3: chromosome ID; 
  Column 4: number of blocks overlapped with chr1.9; 
  Column 5: the first subject block ENSGALT00000015914_GALGA.2 overlapped with chr1.9, 153 is its own size, 153 is the overlapped size; 36125 is overlap-strat-site; 36277 is overlap-end-site;
  Column 6: same form of column 5.
	...
	foreach  my $chr (sort keys %$Ref_hp) {
		...
		for (my $i=0; $i<@ref_chr; $i++) {
			...
			for (my $j=$pre_pos; $j<@pre_chr; $j++) {
				...
				my ($overlap_size,$s,$e) = overlap_size($pre_chr[$j],$ref_chr[$i]);
				
				push @overlap,"$pre_chr[$j][2],$pre_size,$overlap_size,$s,$e";
			}
...
sub overlap_size {
	...
	my $start = ($block1_p->[0] > $block2_p->[0]) ?  $block1_p->[0] : $block2_p->[0];
	my $end   = ($block1_p->[1] < $block2_p->[1]) ?  $block1_p->[1] : $block2_p->[1];
	return ($overlap_size,$start,$end);
}
