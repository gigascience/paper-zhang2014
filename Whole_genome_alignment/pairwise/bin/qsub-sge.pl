...
=head1 Version
  ...
  Version: 9.0,  Date: 2010-08-12 (modified by Liu Shiping,liushiping@genomics.org.cn)
...
while (@Shell) {
	...
	my $job_cmd = "qsub -cwd -S /bin/sh -P tc_fun -q st_tc.q ";  ## -l h_vmem=16G,s_core=8 
	...
	$qsub_cycle++;
	if($qsub_cycle > 100){
		print OUT "\n\nProgram stopped because the reqsub cycle number has reached 100, the following jobs unfinished:\n";
		foreach my $job_id (sort keys %Error) {
			my $shell_file = $Error{$job_id};
